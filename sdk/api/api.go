...
func (api *Api) SetOrm(connStr string) *Api {
	yorm, err := db.NewWithDbms(connStr)
	if err != nil {
		api.AddError(fmt.Errorf("set orm failed, dns: %s", connStr))
	}
	api.Orm = &yorm
	return api
}
...
func (api *Api) RegisterRouters(routerPrefix string, routers ...RouterEntry) {
	api.Routers[routerPrefix] = append(api.Routers[routerPrefix], routers...)
	rg := api.Engine.Group(routerPrefix)
