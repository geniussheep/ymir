...
func (l *zaplog) Logw(level logger.Level, msg string, cfields ...interface{}) {
	l.RLock()
	data := make([]zap.Field, 0, len(l.fields))
	for k, v := range l.fields {
		data = append(data, zap.Any(k, v))
	...
	switch lvl {
	case zap.DebugLevel:
		l.zap.With(data...).Sugar().Debugw(msg, cfields...)
	case zap.InfoLevel:
		l.zap.With(data...).Sugar().Infow(msg, cfields...)
	case zap.WarnLevel:
		l.zap.With(data...).Sugar().Warnw(msg, cfields...)
	case zap.ErrorLevel:
		l.zap.With(data...).Sugar().Errorw(msg, cfields...)
	case zap.FatalLevel:
		l.zap.With(data...).Sugar().Fatalw(msg, cfields...)
	}
}
